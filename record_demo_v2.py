#!/usr/bin/env python3
"""
Lumen Professional Demo Recording Script V2
Improved version with proper wait times for analysis completion
"""
...
async def wait_for_analysis_complete(page, max_wait=20):
    """Wait for analysis to complete by checking for results"""
    print("   ⏳ Waiting for analysis...")
    waited = 0
    while waited < max_wait:
        # Check if results/expanders are visible
...
async def record_demo():
    ...
    print("🎬 Starting Lumen Demo Recording V2...")
    print("=" * 60)
    ...
    print("⏳ Waiting for app to initialize (12 seconds)...")
    ...
    try:
        async with async_playwright() as p:
            ...
            # Navigate to app
            print("📍 Scene 1: Navigate to app (0-5s)")
            await page.goto('http://localhost:8501', wait_until='networkidle')
            await asyncio.sleep(4)
            
            # Upload file
            print("📤 Scene 2: Upload file (5-8s)")
            ...
            print("⚙️  Scene 3: Analysis in progress (8-25s)")
            analysis_complete = await wait_for_analysis_complete(page, max_wait=20)
            
            if not analysis_complete:
                print("   ⚠️  Analysis did not complete in time")
            
            # Extra time for UI to stabilize
            await asyncio.sleep(2)
            
            # Scroll to view results
            print("📊 Scene 4: View results (25-28s)")
            await page.evaluate("window.scrollTo({ top: 300, behavior: 'smooth' })")
            await asyncio.sleep(2)
            
            # Click on risk item
            print("🔍 Scene 5: Examine risk details (28-32s)")
            ...
            print("📋 Scene 6: Review details (32-35s)")
            await page.evaluate("window.scrollTo({ top: 500, behavior: 'smooth' })")
            await asyncio.sleep(2)
            
            # Final pause
            print("✨ Scene 7: Final frame (35-36s)")
            await asyncio.sleep(1)
...
async def main():
    ...
    print()
    print("=" * 60)
    print("  LUMEN PROFESSIONAL DEMO RECORDER V2")
    print("  Improved with proper wait times")
    print("=" * 60)
    print()
